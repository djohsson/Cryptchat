...
class DiffieHellman():
    ...
    def __init__(self, group=standardgroup):
        validgroups = [5, 14, 16, 17, 18]
        if group not in validgroups:
            print("Group is not valid, switching to '%d'" % standardgroup)
            group = standardgroup

        self.generator = 2
        self.prime = self.getprime(group)
        self.keysize = self.getkeysize(group)

        self.privatekey = self.genprivatekey()
        self.publickey = self.genpublickey()

    def getprime(self, group=standardgroup):
        primes = {
        ...
        if group in primes.keys():
            ...
            return primes[standardgroup]

    def getkeysize(self, group):
        keysizes = {
        5:  240,
        14: 320,
        16: 480,
        ...
        if group in keysizes.keys():
            return keysizes[group]
        else:
            print("Group is not valid, returning value for group '%d'" % standardgroup)
